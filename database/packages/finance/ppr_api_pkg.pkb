CREATE OR REPLACE PACKAGE BODY ppr_api_pkg AS
  ...
  EXCEPTION WHEN ppr_service_pkg.e_not_found THEN o_status:=404;o_body:=core_response_pkg.build_known_error('BEX-PPR-001',core_error_pkg.c_category_not_found,'O provedor informado nao foi encontrado.');WHEN OTHERS THEN o_status:=500;o_body:=core_response_pkg.build_technical_error;END;
  ...
  BEGIN r:=ppr_service_pkg.list_active;i:=r.FIRST;WHILE i IS NOT NULL LOOP core_json_pkg.append_element(a,js(r(i)));i:=r.NEXT(i);END LOOP;
    o_body:=core_response_pkg.build_success(a);o_status:=200;EXCEPTION WHEN OTHERS THEN o_status:=500;o_body:=core_response_pkg.build_technical_error;END;
END ppr_api_pkg;
/
