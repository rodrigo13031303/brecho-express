...
DECLARE a BOOLEAN;i NUMBER;p VARCHAR2(32);s PLS_INTEGER;b CLOB;BEGIN ord_runtime_pkg.begin_authenticated_request(:authorization,a,i,p,s,b);IF a THEN plb_api_pkg.create_banner(:body,i,s,b);END IF;:status_code:=s;ord_runtime_pkg.write_json_response(b);ord_runtime_pkg.clear_request_context;EXCEPTION WHEN OTHERS THEN ROLLBACK;ord_runtime_pkg.clear_request_context;RAISE;END;~');
...
DECLARE a BOOLEAN;i NUMBER;p VARCHAR2(32);s PLS_INTEGER;b CLOB;BEGIN ord_runtime_pkg.begin_authenticated_request(:authorization,a,i,p,s,b);IF a THEN plb_api_pkg.update_banner(:bannerPublicId,:body,i,s,b);END IF;:status_code:=s;ord_runtime_pkg.write_json_response(b);ord_runtime_pkg.clear_request_context;EXCEPTION WHEN OTHERS THEN ROLLBACK;ord_runtime_pkg.clear_request_context;RAISE;END;~');
