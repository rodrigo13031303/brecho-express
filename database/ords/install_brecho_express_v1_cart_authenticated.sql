...
~'
  ...
  ORDS.DEFINE_TEMPLATE(
    p_module_name => 'brecho-express-v1',
    p_pattern => 'cart/:cartPublicId/checkout', p_etag_type => 'NONE'
  );
  ORDS.DEFINE_HANDLER(
    p_module_name => 'brecho-express-v1',
    p_pattern => 'cart/:cartPublicId/checkout', p_method => 'POST',
    p_source_type => ORDS.SOURCE_TYPE_PLSQL,
    p_mimes_allowed => 'application/json',
    p_source => q'~
DECLARE
...
BEGIN
  ...
  :trace_id:=core_context_pkg.trace_id();
  IF l_authenticated THEN
    pur_api_pkg.checkout(:cartPublicId,l_account_id,l_status,l_body);
  END IF;
  :status_code:=l_status;ord_runtime_pkg.write_json_response(l_body);
  ord_runtime_pkg.clear_request_context;
EXCEPTION WHEN OTHERS THEN
...
~'
  ...
  define_auth_parameter('cart/:cartPublicId/checkout','POST');
  COMMIT;
END;
/
