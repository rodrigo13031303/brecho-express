...
PROMPT Installing authenticated address and delivery endpoints...
...
BEGIN
  ORDS.DEFINE_TEMPLATE(
    p_module_name=>'brecho-express-v1',p_pattern=>'addresses',
    p_etag_type=>'NONE'
  );
  ORDS.DEFINE_HANDLER(
    p_module_name=>'brecho-express-v1',p_pattern=>'addresses',p_method=>'GET',
...
BEGIN
  ...
  IF a THEN adr_api_pkg.list_addresses(account_id,status,body);END IF;
...
END;~'
  ...
  auth('addresses','GET');
  ORDS.DEFINE_HANDLER(
    p_module_name=>'brecho-express-v1',p_pattern=>'addresses',p_method=>'POST',
    p_source_type=>ORDS.SOURCE_TYPE_PLSQL,p_mimes_allowed=>'application/json',
    p_source=>q'~
...
BEGIN
  ord_runtime_pkg.begin_authenticated_request(:authorization,a,account_id,s,status,body);
  :trace_id:=core_context_pkg.trace_id();
  IF a THEN adr_api_pkg.create_address(request_body,account_id,status,body);END IF;
  :status_code:=status;ord_runtime_pkg.write_json_response(body);
  ord_runtime_pkg.clear_request_context;
EXCEPTION WHEN OTHERS THEN ROLLBACK;ord_runtime_pkg.clear_request_context;RAISE;
END;~'
  ...
  ORDS.DEFINE_TEMPLATE(
    ...
    p_pattern=>'purchase-requests/:requestPublicId/delivery-address',
    p_etag_type=>'NONE'
  );
  ORDS.DEFINE_HANDLER(
    p_module_name=>'brecho-express-v1',
...
BEGIN
  ord_runtime_pkg.begin_authenticated_request(:authorization,a,account_id,s,status,body);
  :trace_id:=core_context_pkg.trace_id();
  IF a THEN pdl_api_pkg.get_delivery(:requestPublicId,account_id,status,body);END IF;
  :status_code:=status;ord_runtime_pkg.write_json_response(body);
  ord_runtime_pkg.clear_request_context;
EXCEPTION WHEN OTHERS THEN ROLLBACK;ord_runtime_pkg.clear_request_context;RAISE;
END;~'
  );
  auth('purchase-requests/:requestPublicId/delivery-address','GET');
  ORDS.DEFINE_HANDLER(
    p_module_name=>'brecho-express-v1',
    p_pattern=>'purchase-requests/:requestPublicId/delivery-address',
...
BEGIN
  ord_runtime_pkg.begin_authenticated_request(:authorization,a,account_id,s,status,body);
  :trace_id:=core_context_pkg.trace_id();
  IF a THEN pdl_api_pkg.select_address(:requestPublicId,request_body,account_id,status,body);END IF;
  :status_code:=status;ord_runtime_pkg.write_json_response(body);
  ord_runtime_pkg.clear_request_context;
EXCEPTION WHEN OTHERS THEN ROLLBACK;ord_runtime_pkg.clear_request_context;RAISE;
END;~'
  );
  auth('purchase-requests/:requestPublicId/delivery-address','PUT');
  COMMIT;
END;
/

PROMPT Authenticated address and delivery endpoints installed.
