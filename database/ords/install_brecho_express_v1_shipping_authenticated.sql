...
PROMPT Installing authenticated shipping quote endpoints...
...
BEGIN
  ORDS.DEFINE_TEMPLATE(
    ...
    p_pattern=>'purchase-requests/:requestPublicId/shipping-options',
    p_etag_type=>'NONE'
  );
  ORDS.DEFINE_HANDLER(
    p_module_name=>'brecho-express-v1',
...
BEGIN
  ...
  IF a THEN psq_api_pkg.quote(:requestPublicId,account_id,status,body);END IF;
...
END;~'
  ...
  auth('purchase-requests/:requestPublicId/shipping-options','GET');
  ORDS.DEFINE_HANDLER(
    p_module_name=>'brecho-express-v1',
    p_pattern=>'purchase-requests/:requestPublicId/shipping-options',
...
BEGIN
  ord_runtime_pkg.begin_authenticated_request(:authorization,a,account_id,s,status,body);
  :trace_id:=core_context_pkg.trace_id();
  IF a THEN psq_api_pkg.select_option(:requestPublicId,request_body,account_id,status,body);END IF;
  :status_code:=status;ord_runtime_pkg.write_json_response(body);
  ord_runtime_pkg.clear_request_context;
EXCEPTION WHEN OTHERS THEN ROLLBACK;ord_runtime_pkg.clear_request_context;RAISE;
END;~'
  );
  auth('purchase-requests/:requestPublicId/shipping-options','PUT');
  COMMIT;
END;
/

PROMPT Authenticated shipping quote endpoints installed.
