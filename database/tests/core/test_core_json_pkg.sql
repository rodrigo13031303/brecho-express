...
DECLARE
  ...
  PROCEDURE test_61_parse_object IS l_object JSON_OBJECT_T;
  BEGIN start_test('parse_object aceita objeto'); l_object:=core_json_pkg.parse_object('{"name":"Brecho"}');
    assert_equals(l_object.get_string('name'),'Brecho','Objeto incorreto.'); pass; END;
  PROCEDURE test_62_parse_rejects_null IS l_object JSON_OBJECT_T;l_raised BOOLEAN:=FALSE;
  BEGIN start_test('parse_object rejeita body NULL');BEGIN l_object:=core_json_pkg.parse_object(NULL);
    EXCEPTION WHEN core_json_pkg.e_request_body_required THEN l_raised:=TRUE;END;
    assert_raises(l_raised,'Body NULL deveria falhar.');pass;END;
  PROCEDURE test_63_parse_rejects_invalid IS l_object JSON_OBJECT_T;l_raised BOOLEAN:=FALSE;
  BEGIN start_test('parse_object rejeita JSON invalido');BEGIN l_object:=core_json_pkg.parse_object('{');
    EXCEPTION WHEN core_json_pkg.e_invalid_json THEN l_raised:=TRUE;END;
    assert_raises(l_raised,'JSON invalido deveria falhar.');pass;END;
  PROCEDURE test_64_parse_rejects_array IS l_object JSON_OBJECT_T;l_raised BOOLEAN:=FALSE;
  BEGIN start_test('parse_object rejeita array');BEGIN l_object:=core_json_pkg.parse_object('[]');
    EXCEPTION WHEN core_json_pkg.e_json_object_required THEN l_raised:=TRUE;END;
    assert_raises(l_raised,'Array deveria falhar.');pass;END;
  PROCEDURE test_65_required_values IS l_object JSON_OBJECT_T;l_flag BOOLEAN;
  BEGIN start_test('leitores obrigatorios preservam tipos');l_object:=core_json_pkg.parse_object('{"name":"A","value":2,"flag":true}');
    assert_equals(core_json_pkg.required_string(l_object,'name'),'A','String incorreta.');
    assert_number_equals(core_json_pkg.required_number(l_object,'value'),2,'Numero incorreto.');
    l_flag:=core_json_pkg.required_boolean(l_object,'flag');assert_true(l_flag,'Boolean incorreto.');pass;END;
  PROCEDURE test_66_required_missing IS l_object JSON_OBJECT_T;l_value VARCHAR2(10);l_raised BOOLEAN:=FALSE;
  BEGIN start_test('leitor obrigatorio rejeita ausencia');l_object:=JSON_OBJECT_T();BEGIN l_value:=core_json_pkg.required_string(l_object,'name');
    EXCEPTION WHEN core_json_pkg.e_required_attribute THEN l_raised:=TRUE;END;
    assert_raises(l_raised,'Ausencia deveria falhar.');pass;END;
  PROCEDURE test_67_required_type IS l_object JSON_OBJECT_T;l_value VARCHAR2(10);l_raised BOOLEAN:=FALSE;
  BEGIN start_test('leitor obrigatorio rejeita tipo');l_object:=core_json_pkg.parse_object('{"name":1}');BEGIN l_value:=core_json_pkg.required_string(l_object,'name');
    EXCEPTION WHEN core_json_pkg.e_invalid_attribute_type THEN l_raised:=TRUE;END;
    assert_raises(l_raised,'Tipo invalido deveria falhar.');pass;END;
  PROCEDURE test_68_optional_and_array IS l_object JSON_OBJECT_T;l_array JSON_ARRAY_T;
  BEGIN start_test('leitores opcionais e array');l_object:=core_json_pkg.parse_object('{"items":[]}');
    assert_true(core_json_pkg.optional_string(l_object,'description') IS NULL,'Opcional ausente deve ser NULL.');
    l_array:=core_json_pkg.required_array(l_object,'items');assert_number_equals(l_array.get_size,0,'Array incorreto.');pass;END;
  PROCEDURE test_69_allowed_attributes IS l_object JSON_OBJECT_T;
  BEGIN start_test('campos permitidos sao aceitos');l_object:=core_json_pkg.parse_object('{"name":"A","active":true}');
    core_json_pkg.assert_allowed_attributes(l_object,'name,active');pass;END;
  PROCEDURE test_70_unknown_attribute IS l_object JSON_OBJECT_T;l_raised BOOLEAN:=FALSE;
  BEGIN start_test('campo desconhecido e rejeitado');l_object:=core_json_pkg.parse_object('{"unexpected":1}');
    BEGIN core_json_pkg.assert_allowed_attributes(l_object,'name');
    EXCEPTION WHEN core_json_pkg.e_unknown_attribute THEN l_raised:=TRUE;END;
    assert_raises(l_raised,'Campo desconhecido deveria falhar.');pass;END;
  PROCEDURE test_71_required_timestamp IS l_object JSON_OBJECT_T;l_value TIMESTAMP;
  BEGIN start_test('timestamp exato e convertido');l_object:=core_json_pkg.parse_object('{"at":"2026-07-26T12:30:45.123"}');
    l_value:=core_json_pkg.required_timestamp(l_object,'at','YYYY-MM-DD"T"HH24:MI:SS.FF');
    assert_equals(TO_CHAR(l_value,'YYYY-MM-DD HH24:MI:SS.FF3'),'2026-07-26 12:30:45.123','Timestamp incorreto.');pass;END;
  PROCEDURE test_72_invalid_timestamp IS l_object JSON_OBJECT_T;l_value TIMESTAMP;l_raised BOOLEAN:=FALSE;
  BEGIN start_test('timestamp invalido e rejeitado');l_object:=core_json_pkg.parse_object('{"at":"26/07/2026"}');
    BEGIN l_value:=core_json_pkg.required_timestamp(l_object,'at','YYYY-MM-DD"T"HH24:MI:SS.FF');
    EXCEPTION WHEN core_json_pkg.e_invalid_temporal_value THEN l_raised:=TRUE;END;
    assert_raises(l_raised,'Timestamp invalido deveria falhar.');pass;END;
  PROCEDURE test_73_required_timestamp_tz IS l_object JSON_OBJECT_T;l_value TIMESTAMP WITH TIME ZONE;
  BEGIN start_test('timestamp com fuso e convertido');l_object:=core_json_pkg.parse_object('{"at":"2026-07-26T12:30:45-03:00"}');
    l_value:=core_json_pkg.required_timestamp_tz(l_object,'at','YYYY-MM-DD"T"HH24:MI:SSTZH:TZM');
    assert_equals(TO_CHAR(l_value,'TZH:TZM'),'-03:00','Fuso incorreto.');pass;END;
BEGIN
  ...
  test_61_parse_object;
  test_62_parse_rejects_null;
  test_63_parse_rejects_invalid;
  test_64_parse_rejects_array;
  test_65_required_values;
  test_66_required_missing;
  test_67_required_type;
  test_68_optional_and_array;
  test_69_allowed_attributes;
  test_70_unknown_attribute;
  test_71_required_timestamp;
  test_72_invalid_timestamp;
  test_73_required_timestamp_tz;

  DBMS_OUTPUT.PUT_LINE('SUCCESS - CORE_JSON_PKG (73 testes)');
EXCEPTION
  WHEN OTHERS THEN
