...
(
  ...
  CAT_SORT_ORDER  NUMBER(9) DEFAULT 0 NOT NULL,
  CAT_ACCEPTS_PRODUCTS NUMBER(1) DEFAULT 1 NOT NULL,
  ...
  CONSTRAINT FK_CATEGORY_PARENT FOREIGN KEY (CAT_PARENT_ID)
    REFERENCES BEX_CATEGORY (CAT_ID),
  CONSTRAINT CK_CATEGORY_SORT_ORDER CHECK (CAT_SORT_ORDER BETWEEN 0 AND 999999999),
  CONSTRAINT CK_CATEGORY_ACCEPTS_PRODUCTS CHECK (CAT_ACCEPTS_PRODUCTS IN (0,1))
);
...
CREATE INDEX IDX_CATEGORY_PARENT
  ON BEX_CATEGORY (CAT_PARENT_ID, CAT_SORT_ORDER, CAT_NAME);
...
BEGIN
  ...
  SELECT COUNT(*) INTO l_constraint_count
   ...
   WHERE TABLE_NAME = 'BEX_CATEGORY'
     AND (
       ...
       OR (CONSTRAINT_NAME = 'FK_CATEGORY_PARENT' AND CONSTRAINT_TYPE = 'R')
       OR (CONSTRAINT_NAME = 'CK_CATEGORY_SORT_ORDER' AND CONSTRAINT_TYPE = 'C')
       OR (CONSTRAINT_NAME = 'CK_CATEGORY_ACCEPTS_PRODUCTS' AND CONSTRAINT_TYPE = 'C')
     );
  ...
  IF l_table_count <> 1
     OR l_column_count <> 13
     OR l_constraint_count < 7
